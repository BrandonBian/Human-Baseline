...
class AssemblyGraph():
    ...
    def get_graph_data(self):
        ...
        # TODO: further processing of the graph links (edges)
        self.process_graph_links()

        return self.graph_nodes, self.graph_links, self.graph_edges
    ...
    def get_graph_networkx(self):
        ...
        graph_data["nodes"], graph_data["links"], graph_data["edges"] = self.get_graph_data()
    ...
    def process_graph_links(self):

        link_ids = set([f["id"] for f in self.graph_links])
    ...
    def populate_graph_joint_links(self):
        ...
        if self.assembly_data["joints"] == None:
            return
        for joint_uuid, joint in self.assembly_data["joints"].items():  # TODO: Question - no "joints" in dataset?
...
def assembly2graph(args):
    ...
    for input_file in input_files:
        ...
        ag.export_graph_json(json_file)  # Saves as another json file
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", type=str, required=True, help="Input folder/file with data"
    )
    parser.add_argument(
        "--output", type=str, default="data", help="Output folder for results"
    )
    parser.add_argument(
        "--threads", type=int, default=8, help="Number of threads to use for multiprocessing [default: 8]"
    )
